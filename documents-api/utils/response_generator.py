...
def build_document_context(documents: List[Dict[str, Any]], max_docs: int = 10) -> Tuple[str, Dict[str, str]]:
    """
    ...
    for i, doc in enumerate(documents[:max_docs], 1):
        ...
        LOGGER.info(f"📄 Document {i}: {doc_title} (ID: {doc_id}, Text length: {len(doc_text)})")
        
        # Use description as fallback if text is empty
        if not doc_text:
            LOGGER.warning(f"⚠️ Document {doc_title} (ID: {doc_id}) has empty text content")
...
def _build_metadata_string(doc_metadata: Dict[str, Any]) -> str:
    ...
    if doc_metadata.get('documentNumberLabel'):
        metadata_parts.append(f"Document Number Label: {doc_metadata.get('documentNumberLabel')}")
...
async def stream_response_from_documents(
    ...
    If the offset is greater than or equal to the number of available documents, the function yields an empty result set and appropriate pagination metadata.
    
    Args:
        query: User's question
        documents: All relevant documents from search
        previous_chats: Previous conversation context
        model: AI model for generating response
    ...
    if not documents:
        ...
        yield "__PAGINATION__:" + json.dumps({"total": 0, "offset": offset, "limit": limit, "hasMore": False}) + "\n"
        yield "__TOKEN_USAGE__:" + json.dumps({"input_tokens": 0, "output_tokens": 0}) + "\n"
    ...
    LOGGER.info(f"📑 Pagination: offset={offset}, limit={limit}, total={total_documents}, showing={len(paginated_documents)}, hasMore={has_more}")
    
    if not paginated_documents:
        # Consistent order: PAGINATION first, then TOKEN_USAGE
        yield "__PAGINATION__:" + json.dumps({"total": total_documents, "offset": offset, "limit": limit, "hasMore": False}) + "\n"
        yield "__TOKEN_USAGE__:" + json.dumps({"input_tokens": 0, "output_tokens": 0}) + "\n"
        yield "No more documents to show for this query."
        return
    
    # Build context from paginated documents
    context_text, document_map = build_document_context(paginated_documents, max_docs=limit)
    
    # Get first document metadata for prompt example
    first_doc_metadata = paginated_documents[0].get("metadata", {}) if paginated_documents else {}
    ...
    try:
        ...
        for chunk in response:
            ...
            if hasattr(chunk, 'usage_metadata') and chunk.usage_metadata:
                new_input_tokens = getattr(chunk.usage_metadata, 'prompt_token_count', None)
                if new_input_tokens is not None:
                    input_tokens = new_input_tokens
                new_output_tokens = getattr(chunk.usage_metadata, 'candidates_token_count', None)
                if new_output_tokens is not None:
                    output_tokens = new_output_tokens
            
            # Yield the text chunk
            if hasattr(chunk, 'text') and chunk.text:
                # Send token usage before first content chunk
                if not token_usage_sent:
                    # We'll send final token usage at the end instead
                    token_usage_sent = True
                yield chunk.text
        
        # Consistent order: PAGINATION first, then TOKEN_USAGE
        yield "\n__PAGINATION__:" + json.dumps({
        ...
        yield "__TOKEN_USAGE__:" + json.dumps({"input_tokens": input_tokens, "output_tokens": output_tokens}) + "\n"
        
    except Exception as e:
        LOGGER.error(f"Error streaming response from documents: {e}")
        # Consistent order: PAGINATION first, then TOKEN_USAGE
        yield "__PAGINATION__:" + json.dumps({"total": total_documents, "offset": offset, "limit": limit, "hasMore": has_more}) + "\n"
        yield "__TOKEN_USAGE__:" + json.dumps({"input_tokens": 0, "output_tokens": 0}) + "\n"
